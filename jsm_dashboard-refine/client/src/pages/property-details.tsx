...
// Import the CustomButton component from the "components" module
import { CustomButton } from "components";

// Define a functional component called "PropertyDetails"
const PropertyDetails = () => {
  // Get a function for programmatic navigation between routes
  const navigate = useNavigate();
  // Get the current user's identity
  const { data: user } = useGetIdentity();
  // Get the record ID from the URL parameters
  const { id } = useParams();
  // Get a function for deleting the record with the given ID
  const { mutate } = useDelete();
  // Get the record with the given ID
  const { queryResult } = useShow();

  // Destructure the "data", "isLoading", and "isError" properties from the "queryResult" object
          ...
          >
            ...
            <Stack mt={2} justifyContent="center" alignItems="center" textAlign="center">
              ...
              <Stack mt="15px" direction="row" alignItems="center" gap={1}>
                <Place sx={{ color: '#808191' }} />
                <Typography fontSize={14} fontWeight={400} color="#808191">San Francisco, USA</Typography>
              </Stack>
